...
4a. In this edition, an `EXPLOIT CHAINS` section derived from `chains.json`, placed after the confirmed findings and before `NEEDS VALIDATION`. Give each chain its verdict, entry principal, ordered links by fingerprint and title, composite result, and break point, and state that its constituent findings are already counted above. An `open` chain carries its blockers and no severity; a `refuted` chain is retained in `chains.json` and not reported. When no chain was assembled, say so and why.
...
Item 1 additionally states, in this edition: the deployment intent, `history_status`, structural-index coverage, the sandbox tier actually available, and any `unknown` calibration outcome.
